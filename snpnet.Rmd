...
# Are we doing everything right order-wise?
atleastonce <- unique(unlist(sapply(p_values, function(p) p$ID)))

p_vals <- lapply(p_values, function(x) {
...
names(Q_gamma) <- rownames(p_vals) <- atleastonce

save(p_values, p_vals, Q_gamma, file = "sample_splitting_results.RData")
