...
Ecologists and genomicists are interested in understanding how variation in the genetic architecture of populations of species in order to determine the traits which drive the fitness of a species. To do so, they study single nucleotide polymorphisms (SNPs) common across a population, which represent a variation at a specific position in a DNA sequence.
...
For this project, we focus on studying the genetic architecture of the house sparrows, analyzing which SNPs have a significant effect on the wing length of the sparrows. Genomic data is usually characterized by an incredibly large numbers of SNP effects, which makes finding the statistical effects of them on the response difficult. As a result, we use regularization techniques to find an optimal penalty term for our model. We then use sample splitting techniques models to compute the *p-values* and associated confidence intervalsfor the SNPs.
