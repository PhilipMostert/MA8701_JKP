...
```{r load results, echo = FALSE, message=FALSE}

# We are "cheating a bit":
# Load results here so we can knit without slowly rerunning the analysis
library(glmnet)
...
For this project, we focus on studying the genetic architecture of the house sparrows, analyzing which SNPs (if any) have a significant additive effect on the wing length of the sparrows (at age 1). Genomic data is usually characterized by a large numbers of SNPs, which makes finding their effects on the response difficult. As a result, we will here use lasso regularization and multiple sample splitting techniques to compute the *p-values* for the effects of the SNPs. We will also correct for the effects of sex, hatch year, island of measurement and inbreeding coefficient (`FGRM`), and these non-SNP covariates will not be penalized in the lasso.
...
We will use the full SNP data set, in the files `Genotypes.map` and `Genotypes.ped`. We first prepare the data using PLINK v2. To run our code, you need to download the genomic data and phenotype data from @DataUsed and the program `plink2` from [here](https://www.cog-genomics.org/plink/2.0/) and put it in your working folder. We do some standard quality control steps (remove SNPs with minor allele frequency lower than $0.01$, remove SNPs missing in more than $10\%$ of individuals and remove individuals missing more than $5\%$ of the SNPs). These steps are performed to avoid including markers or individuals whose genotyping was unsuccessful in the analysis [@QC]. We also change the dataset to the input format that `snpnet` needs.
...
```{r phenotype, message = FALSE, warning  = FALSE}
# Load the data
library(dplyr)
...
# Manually make dummy variables since snpnet and plink are not as smart as lm
...
Looking at the dimensions of our data, we see that we have 1796 observations of wing length, and 23 non-SNP covariates in the model.
...
Various descriptive plots of the response variable are given below.
...
ggplot(data = df2, aes(x = age1wing)) +
  ...
  ggtitle('Histogram of the age 1 wing lengths (mm)')

ggplot(data = df2, aes(y = age1wing, x = factor(gen_sex))) +
  geom_boxplot() +
  xlab("Genetic sex") +
  ylab("Age 1 wing length")
...
ggplot(data = df2, aes(y = age1wing, x = factor(fiflok))) +
  geom_boxplot() +
  xlab("Island of measurement") +
  ylab("Age 1 wing length")

```

The mean of the age 1 wing lengths is `r sprintf("%.2f", mean(df2$age1wing))`mm with a standard deviation of `r sprintf("%.2f", sd(df2$age1wing))`mm.

Next, we check the linear model assumptions for the base model with no SNPs included as covariates.

```{r model_checking}
# MLR on full data set, no SNPs.
# gen_sex, all_hatchyears, fiflok (island of measurement) are categorical, with 
# 2, 16 and 8 levels, respectively, while FGRM (inbreeding coef.) is continuous
non_snp_mod <- lm(get(response) ~ FGRM + gen_sex + all_hatchyears + fiflok,
                  data = df2)

# Check model assumptions
plot(non_snp_mod, which = c(1, 2))
```

Based on the residuals vs. fitted plot and the normal QQ-plot, the linear regression assumptions seem to be fine The reason we get two "bands" of fitted values in the residual vs. fitted plot is that the male sparrows generally have longer wings than the females, as seen in an earlier plot. The non-SNP covariates give an $R^2$ of `r sprintf("%.2f", summary(non_snp_mod)$r.squared)`.
...
3.  Use the variables in $\hat{S}\left(I_1\right)$ to conduct estimation using data $I_2$ with least-squares, and find $m$ p-values for the estimated coefficients. For missing genotypes, we use replace the missing SNP count with two times the allele frequency, in other words mean imputation.

4.  Correct the $j^{th}$ raw p-values for multiple testing using: $P_{corr, j} = min\left(P_j * \left|\hat{S}\left(I_1\right)\right|,1 \right)$.
...
3.  Finally, correct for trying many different values of $\gamma$ by multiplying by $(1 - \log(\gamma_\text{min}))$, to obtain the final FWER-adjusted p-values: $$
...
When performing lasso-regression we need to choose a value for the penalization parameter $\lambda$. We use 10-fold cross-validation for this. Preferably the cross-validation loop should happen inside the multiple sample-splitting procedure to generate a new $\lambda$ for each split to avoid data leakage, but in the interest of computational time we pre-compute a global $\lambda$ to be used in all splits. Below is the code for the cross-validation for $\lambda$.

```{r checkthings, echo = FALSE, eval = FALSE}
...
full_model <- snpnet(genotype.pfile = "sparrow_snps",
                     ...
                     phenotype = response,
                     # # Add fixed unpenalized effects here
                     covariates = covs,
...
Below we perform the multiple sample splitting and generate FWER-adjusted p-values.

```{r ss, echo=TRUE, eval = FALSE}
...
# All SNPs with non-zero beta in at least one split
...
P_val_final <- pmin(1, apply(as.data.frame(Q_gamma), MARGIN = 1, FUN = min) *
                      (1 - log(gamma_min)))
...
The final FWER-adjusted p-values for all SNP-effects equal $1$, we essentially find a null result. Below we provide some miscellaneous plots from the results.
...
ggplot(map = aes(x = 1:length(opt_betas), y = opt_betas)) +
  geom_point() +
  ylab("beta") + 
  xlab("index") +
  ggtitle("Non-zero betas in the full Lasso model with optimal lambda")
```

Before the Bonferroni-correction, the number of raw p-values that are $<1$ are around 300-500 in each split (for the first few iterations). After the Bonferroni-correction, the number of corrected p-values that are $<1$ in a given split is shown in the figure below.

```{r num_beta_plot}
num_non1 <- apply(p_values_split_combined, 2, function(x) sum(x != 1))

ggplot(map = aes(x = as.factor(num_non1))) +
  geom_bar() +
  ggtitle("Number of p-values that are <1 in each split (after Bonferroni-correction)") +
  xlab("p-values <1") +
  ylab("Number of splits")
```

So, in a given split `r sprintf("0-%d", max(num_non1))` p-values "survive" the Bonferroni correction. These remaining p-values are also set to $1$ when accounting for the multiple sample-splitting.

## Conclusion

All our computed p-values equal $1$, so we don't find evidence that any of the available SNPs are related to wing length. In other words, a null result. If we had found significant SNPs we would have wanted to verify their effect in an independent test-set, but this is not relevant here due to our null result. This result is not too surprising, due of the "infinitesimal model," which is a standard assumption in quantitative genetics that complex traits are determined by a large number of SNPs, each with a very small contribution to the phenotype. If each SNP has a very small effect, we would presumably need a larger sample size to detect the influence of individual SNPs. The multiple sample-splitting methodology has to our knowledge not been used in evolutionary ecology before, and with our inconclusive results it is hard to say whether it should be. For a more standard GWAS-analysis in evolutionary ecology, see e.g. @lundregan2018inferences.
